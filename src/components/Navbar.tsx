...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 w-full bg-nordic-gradient text-white shadow-lg z-50 top-nav-safe">
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-sm border-t border-nordic-blue/30 shadow-lg z-50 bottom-nav-safe">
        <div className="max-w-4xl mx-auto flex justify-around items-center px-4 py-0 w-full">
          {navItems.map((item) => (
            <Tooltip.Root key={item.path} delayDuration={100}>
                  ...
                  className={`flex flex-col items-center justify-center gap-1 px-6 py-3 transition-all duration-200
                    ${
                      location.pathname === item.path
                        ? "text-nordic-blue font-semibold"
                        : "text-gray-500 hover:text-nordic-accent"
                    }`}
                  style={{ minWidth: 60 }}
                >
                  {item.icon}
              ...
              <Tooltip.Portal>
                <Tooltip.Content className="text-xs bg-nordic-dark text-nordic-white px-3 py-1.5 rounded shadow-lg font-medium">
                  {t[item.key]}
                  <Tooltip.Arrow className="fill-nordic-dark" />
                </Tooltip.Content>
              </Tooltip.Portal>
            </Tooltip.Root>
          ))}
